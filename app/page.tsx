...
export default function Pages() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setProgress((oldProgress) => {
        ...
        const diff = 0.5 + Math.random() * 1.5;
  ...
  useEffect(() => {
    if (progress === 100) {
      const navigationTimer = setTimeout(() => {
        router.push('/desktop');
      }, 1000); // Wait for 1 second after reaching 100% before navigating
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white">
      ...
      <Progress
        aria-label="Loading..."
        className="max-w-[15rem] w-full h-1 animate-fade-in"
        classNames={{
            base: 'max-w-xs',
            track: 'bg-transparent',
            indicator: 'bg-white transition-all duration-200 ease-out',
        }}
        value={progress}
      />
    </div>
